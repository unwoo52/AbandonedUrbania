...
namespace Lightbug.CharacterControllerPro.Demo
{
    ...
    public class AIFollowBehaviour : CharacterAIBehaviour
    {
        ...
        public void DisableFollowTarget()
        {
            IsTrackingState = false;
            SetFollowTarget(null);
            characterActions.Reset();
        }
        public void SetMoveDir(float horizontal, float vertical)
        {
            SetMovementAction(new Vector3(horizontal,0,vertical));
        }
        void UpdateFollowTargetBehaviour(float dt)
        {
            ...
            {
                Debug.Log("Fail to Nav");
                return;
            }

            for (int i = 0; i < navMeshPath.corners.Length - 1; i++)
            {
                Debug.DrawLine(navMeshPath.corners[i], navMeshPath.corners[i + 1], Color.red, 0.5f);
            }
